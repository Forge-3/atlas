...
use ic_stable_structures::{StableBTreeMap, DefaultMemoryImpl, StableCell};
use std::cell::RefCell;

use crate::config::Config;
use crate::errors::Error;
use crate::state::State;
use crate::task::{TaksId, Task};
...
thread_local! {
   ...
   static OPEN_TASKS_MAP: RefCell<StableBTreeMap<TaksId, Task, VMem>> = RefCell::new(
        StableBTreeMap::init(
            MEMORY_MANAGER.with(|m| m.borrow().get(OPEN_TASKS_MAP_MEMORY_ID)),
        )
    );
    static CLOSED_TASKS_MAP: RefCell<StableBTreeMap<TaksId, Task, VMem>> = RefCell::new(
        StableBTreeMap::init(
...
pub fn insert_task(task_id: TaksId, new_task: Task) -> Result<(), Error> {
