...
pub const SPACE_DEFAULT_CYCLES: u128 = 1_000_000_000_000;
...
impl Storable for Space {
    fn to_bytes(&self) -> Cow<[u8]> {
        let mut buf = vec![];
        minicbor::encode(self, &mut buf).expect("Space encoding should always succeed");
        Cow::Owned(buf)
    }

    fn from_bytes(bytes: Cow<[u8]>) -> Self {
        minicbor::decode(bytes.as_ref())
            .unwrap_or_else(|e| panic!("failed to decode Space bytes {}: {e}", hex::encode(bytes)))
    }
